...
newsave_dir = [save_dir filesep 'pruitt_poster'];
mkdir(newsave_dir);
%% (STATS STRUCT) ====================================================== %%
...
%## FACTORS
meas_names = {'mean_APexc_COV','mean_MLexc_COV','mean_APexc_COV','mean_MLexc_COV'};
meas_ylabel = {'COV (%)','COV (%)','COV (%)','COV (%)'};
% meas_titles = {{'Anteroposterior Excursion';'Coefficient of Variation'},...
%     {'Mediolateral Excursion';'Coefficient of Variation'},...
%     {'Anteroposterior Excursion';'Coefficient of Variation'},...
%     {'Mediolateral Excursion';'Coefficient of Variation'}};
meas_titles = {{'Anteroposterior Excursion';'Coefficient of Variation'},...
    {'Mediolateral Excursion';'Coefficient of Variation'},...
    {''},...
...
xtick_label_gt = {'flat','low','med','high'};
% YLIMS = {[0,90],[0,95]};
YLIMS = {[0,75],[0,47.5],[0,75],[0,47.5]};
%## TEMPS
horiz_shift = 0;
vert_shift = 0;
cnts = 1;
...
%-
conds_keep = {1,2,3,4};
tmp = (cellfun(@(x) table_new_imu.trial_tag_in == categorical(x),conds_keep,'UniformOutput',false));
tmp = any(cat(2,tmp{:}),2);
inds = table_new_imu.(DESIGN_TABLE_VAR) == des_i & tmp &...
    (table_new_imu.(GROUP_TABLE_VAR) == 2 | table_new_imu.(GROUP_TABLE_VAR) == 3);
% inds = table_new_imu.(DESIGN_TABLE_VAR) == des_i & tmp &...
%     table_new_imu.(GROUP_TABLE_VAR) == 1;
tmp_table = table_new_imu(inds,:);
tmp_table = table(categorical(double(string(tmp_table.trial_tag_in))),double(tmp_table.mean_terrain_speed),...
    categorical(string(tmp_table.subj_char)),...
    categorical(string(tmp_table.(GROUP_TABLE_VAR))),tmp_table.(meas_names{meas_i}),...
    'VariableNames',{'trial_char','terrain_sp','subj_char',GROUP_TABLE_VAR,meas_names{meas_i}});
%##
...
%-
mod_out = sprintf('%s~1+trial_char+terrain_sp+(1|subj_char)',meas_names{meas_i});
stats_out = fitlme(tmp_table,mod_out); %'DummyVarCoding','effects');
% pred_terms = stats_out.CoefficientNames;
anova_out = anova(stats_out);
% (stats_out.Rsquared.Ordinary - altstats_out.Rsquared.Ordinary)/(1-stats_out.Rsquared.Ordinary)
...
altmod_out = sprintf('%s~1+terrain_sp+(1|subj_char)',meas_names{meas_i});
...
% alt_f22 = (stats_out.Rsquared.Ordinary - altstats_out.Rsquared.Ordinary)/(1-stats_out.Rsquared.Ordinary);
%##
% mod_out = sprintf('%s~1+trial_char+terrain_sp+(1|subj_char)',meas_names{meas_i});
% stats_out1 = fitlme(tmp_table,mod_out);
% mod_out = sprintf('%s~1+terrain_sp+(1|subj_char)',meas_names{meas_i});
% stats_out2 = fitlme(tmp_table,mod_out);
% mod_out = sprintf('%s~1',meas_names{meas_i});
% stats_out4 = fitlme(tmp_table,mod_out);
% r2ab = (stats_out1.SSR/stats_out4.SST);
% r2a = (stats_out2.SSR/stats_out4.SST);
% alt_f2 = (r2ab-r2a)/(1-r2ab);
%##
anova_p = anova_out.pValue(strcmp(anova_out.Term,'trial_char'));
c2s = stats_out.Coefficients.pValue(strcmp(stats_out.Coefficients.Name,'trial_char_2'));
c3s = stats_out.Coefficients.pValue(strcmp(stats_out.Coefficients.Name,'trial_char_3'));
c4s = stats_out.Coefficients.pValue(strcmp(stats_out.Coefficients.Name,'trial_char_4'));
...
if anova_p > 0.01 && anova_p < 0.05
    str = {sprintf('* Cohens f^2=%0.2g',alt_f2)};
...
[~,~,~,adj_p] = fdr_bh([c2s,c3s,c4s]);
STATS_STRUCT = struct('anova',{{anova_p}},...
    'anova_grp',{{}},...
    'pvals',{{[1,adj_p]}},...
...
VIO_PLOT_STRUCT.ylim = YLIMS{meas_i}; %prc_ylim;
VIO_PLOT_STRUCT.group_lab_yoffset = -.22;
% VIO_PLOT_STRUCT.regresslab_txt_size = 7;
VIO_PLOT_STRUCT.do_combine_groups = true;
% VIO_PLOT_STRUCT.group_labels = {'Older Adults'};
...
prc_ylim = [floor(prctile(tmp_table.(meas_names{meas_i}),1))-floor(std(tmp_table.(meas_names{meas_i}))),...
            ceil(prctile(tmp_table.(meas_names{meas_i}),99))+ceil(std(tmp_table.(meas_names{meas_i})))*2.6];
%-
mod_out = sprintf('%s~1+trial_char+(1|subj_char)',meas_names{meas_i});
stats_out = fitlme(tmp_table,mod_out);
% pred_terms = stats_out.CoefficientNames;
anova_out = anova(stats_out);
% anova_out = anova(tmp_table,mod_out);
% multcompare(anova_out,[""])
% [p,t,anova_out,terms] = anovan(tmp_table.(meas_names{meas_i}),{tmp_table.trial_char},...
%     'sstype',3,'varnames',{'trial_char'},'model','linear','Display','off','continuous',1);
% [comparisons,means,~,gnames] = multcompare(anova_out,'Dimension',[2],...
%     'display','off','Alpha',0.05); % comparisons columns: [pred1,pred2,lowerCI,estimate,upperCI,p-val]
%##
disp(stats_out)
...
% mod_out = sprintf('%s~1+trial_char+(1|subj_char)',meas_names{meas_i});
% stats_out1 = fitlme(tmp_table,mod_out);
% mod_out = sprintf('%s~1+(1|subj_char)',meas_names{meas_i});
% stats_out2 = fitlme(tmp_table,mod_out);
% mod_out = sprintf('%s~1',meas_names{meas_i});
% stats_out4 = fitlme(tmp_table,mod_out);
% r2ab = (stats_out1.SSR/stats_out4.SST);
...
speed_p = anova_out.pValue(strcmp(anova_out.Term,'trial_char')); %stats_out.Coefficients.pValue(2);
speed_r2 = stats_out.Rsquared.Adjusted;
coeffs = [stats_out.Coefficients.Estimate];
speed_xvals = (0:5)*0.25;
if anova_p > 0.01 && anova_p < 0.05
    str = {sprintf('* Cohens f^2=%0.2g\nR^2=%0.2g\nm=%0.2g',alt_f2,speed_r2,coeffs(2))};
elseif anova_p <= 0.01 && anova_p > 0.001
    str = {sprintf('** Cohens f^2=%0.2g\nR^2=%0.2g\nm=%0.2g',alt_f2,speed_r2,coeffs(2))};
elseif anova_p <= 0.001
    str = {sprintf('*** Cohens f^2=%0.2g\nR^2=%0.2g\nm=%0.2g',alt_f2,speed_r2,coeffs(2))};
...
VIO_PLOT_STRUCT.cond_offsets = linspace(-0.35,.4,4);
VIO_PLOT_STRUCT.xlim = [];
VIO_PLOT_STRUCT.ylim = YLIMS{meas_i};
VIO_PLOT_STRUCT.group_lab_yoffset = -.22;
% VIO_PLOT_STRUCT.regresslab_txt_size = 7;
VIO_PLOT_STRUCT.do_combine_groups = true;
VIO_PLOT_STRUCT.ax_position = [AX_INIT_HORIZ+horiz_shift,AX_INIT_VERT+vert_shift,AX_W*IM_RESIZE,AX_H*IM_RESIZE];  %[left bottom width height]
...
R22 = stats_out.SSR/stats_out.SST; % coefficient of determination
alt_f2 = (R22-R21)/(1-R22);
% r2_full = stats_out.Rsquared.Ordinary;
% r2_red = altstats_out.Rsquared.Ordinary;
% alt_f22 = (r2_full-r2_red)/(1-r2_full);
%## probs more accurate f2?
% mod_out = sprintf('%s~1+trial_char+(1|subj_char)',meas_names{meas_i});
% stats_out1 = fitlme(tmp_table,mod_out);
% mod_out = sprintf('%s~1+(1|subj_char)',meas_names{meas_i});
...
% horiz_shift = horiz_shift + AX_W*IM_RESIZE + AX_HORIZ_SHIFT*IM_RESIZE;
%## SAVE
exportgraphics(fig,[newsave_dir filesep 'figure_behavioral_data_pruitt_anova_changf2.tiff'],'Resolution',1000);

%%
save([newsave_dir filesep 'figure_behavioral_data_stats_anova.mat'],'stats_struct')
stats_struct = struct2table(stats_struct);
stats_struct(isundefined(stats_struct.measure_tag),:) = [];
% stats_struct(:,'anova_preds_terms')
writetable(stats_struct,[newsave_dir filesep 'figure_behavioral_data_stats_anova.xlsx'])