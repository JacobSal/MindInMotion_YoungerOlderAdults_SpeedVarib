...
%- (work on finding out how to merge the two tables to have equal
%participates
% keys = {'subj_char','trial_char','subj_cat'};
% for i = 1:length(keys)
% end
% tmp1 = table_new_ls;
% tmp2 = table_new_imu;
% tmp1.collection_team = [];
% tmp2.collection_team = [];
% tmp_tab = join(tmp1,tmp2,'Keys',{'subj_char','trial_char','subj_cat'});
...
% anova(stats_out);
%-
% p1 = coefTest(stats_out,[0,2,0]);
% p2 = coefTest(stats_out,[0,0,1]);
% p3 = coefTest(stats_out,[0,-1,1]);
% [h,crit_p,adj_ci_cvrg,adj_p] = fdr_bh([p1,p2,p3],0.001);
%-
ypred1 = stats_out.Coefficients{1,1}*double(tmp_table.(GROUP_TABLE_VAR)==categorical(1));
ypred2 = (stats_out.Coefficients{2,1}+stats_out.Coefficients{1,2})*double(tmp_table.(GROUP_TABLE_VAR)==categorical(2));
ypred3 = (stats_out.Coefficients{3,1}+stats_out.Coefficients{1,2})*double(tmp_table.(GROUP_TABLE_VAR)==categorical(3));
ypred1(ypred1==0) = [];
ypred2(ypred2==0) = [];
ypred3(ypred3==0) = [];
ypred2 = ypred2 + stats_out.Coefficients{1,1};
ypred3 = ypred3 + +stats_out.Coefficients{1,1};
ypred = [ypred1;ypred2;ypred3];
yactu = tmp_table.(meas_names{meas_i});
resid_var_full = var(sqrt((yactu-ypred).^2));
...
alt_f2null = (R22-R21)/(1-R22);
ypred = (altstats_out.Coefficients{1,1});
yactu = tmp_table.(meas_names{meas_i});
resid_var_null = var(sqrt((yactu-ypred).^2));
%-
R2 = (resid_var_null-resid_var_full)/resid_var_null;
% %- populate struct
...
stats_xlsx(cnts).anova_preds_terms = cell2csv_util(t(:,1));
tmp = t(:,7)';
tmp = tmp(~cellfun(@isempty,tmp));
stats_xlsx(cnts).anova_preds_p = cell2csv_util(tmp);
tmp = t(:,6)';
tmp = tmp(~cellfun(@isempty,tmp));
stats_xlsx(cnts).anova_preds_stat = cell2csv_util(tmp);
tmp = t(:,3)';
tmp = tmp(~cellfun(@isempty,tmp));
stats_xlsx(cnts).anova_preds_df1 = cell2csv_util(tmp(2));
stats_xlsx(cnts).anova_preds_df2 = cell2csv_util(tmp(3));
stats_xlsx(cnts).mod_preds_p = cell2csv_util(stats_out.Coefficients.pValue);
stats_xlsx(cnts).mod_preds_terms = cell2csv_util(stats_out.Coefficients.Properties.RowNames');
...
stats_xlsx(cnts).mod_r2 = stats_out.Rsquared.Adjusted;
stats_xlsx(cnts).norm_test_p = norm_p;
stats_xlsx(cnts).norm_test_h = norm_h;
stats_xlsx(cnts).effect_size = alt_f2null;
...
str = {sprintf('%s F=%0.2g',ast,t{2,6}),...
    '',...
    ''};
...
%## ANOVA
mod_out = sprintf('%s~1+%s+trial_char+(1|subj_char)',meas_names{meas_i},GROUP_TABLE_VAR);
% mod_out = sprintf('%s~1+%s+trial_char+trial_char*%s+(1|subj_char)',meas_names{meas_i},GROUP_TABLE_VAR,GROUP_TABLE_VAR);
stats_out = fitlme(tmp_table,mod_out);
anova_out = anova(stats_out);
% [p1,F1,DF1,DF2] = coefTest(stats_out,[0,1,-1,0,-1,0]); % group 1 compared to group 2
% [p2,F2,DF1,DF2] = coefTest(stats_out,[0,1,0,-1,0,-1]); % group 1 compared to group 3
% [p3,F3,DF1,DF2] = coefTest(stats_out,[0,0,1,-1,1,-1]); % group 2 compared to group 3
% [h,crit_p,adj_ci_cvrg,adj_p] = fdr_bh([p1,p2,p3]);
[p1,F1,DF1,DF2] = coefTest(stats_out,[0,0,-1,0]); % group 1 compared to group 2
[p2,F2,DF1,DF2] = coefTest(stats_out,[0,0,0,-1]); % group 1 compared to group 3
[p3,F3,DF1,DF2] = coefTest(stats_out,[0,0,1,-1]); % group 2 compared to group 3
% [p1,F1,DF1,DF2] = coefTest(stats_out,[-0.25,0.75,-0.25,-0.25]) % group 1 compared to group 2
[h,crit_p,adj_ci_cvrg,adj_p] = fdr_bh([p1,p2,p3]);
multi_comps_p = adj_p;
multi_comps_f = [F1,F2,F3];
multi_comps_t1_t2 = [[1,2];[1,3];[2,3]];
multi_comps_gn = {'intercept',stats_out.Coefficients.Name{3},stats_out.Coefficients.Name{4}};
%## GATHER STATS
%- get effects
...
altstats_out = fitlme(tmp_table,altmod_out);
R21 = altstats_out.SSR/altstats_out.SST; % coefficient of determination
R22 = stats_out.SSR/stats_out.SST; % coefficient of determination
alt_f2null = (R22-R21)/(1-R22);
%- alt 1
altmod_out = sprintf('%s~1+%s+(1|subj_char)',meas_names{meas_i},GROUP_TABLE_VAR);
altstats_out = fitlme(tmp_table,altmod_out);
R21 = altstats_out.SSR/altstats_out.SST; % coefficient of determination
R22 = stats_out.SSR/stats_out.SST; % coefficient of determination
alt_f2t = (R22-R21)/(1-R22);
%- alt 2
altmod_out = sprintf('%s~1+trial_char+(1|subj_char)',meas_names{meas_i});
altstats_out = fitlme(tmp_table,altmod_out);
R21 = altstats_out.SSR/altstats_out.SST; % coefficient of determination
R22 = stats_out.SSR/stats_out.SST; % coefficient of determination
alt_f2g = (R22-R21)/(1-R22);
%-
inds1 = double(tmp_table.(GROUP_TABLE_VAR)==categorical(1));
inds2 = double(tmp_table.(GROUP_TABLE_VAR)==categorical(2));
inds3 = double(tmp_table.(GROUP_TABLE_VAR)==categorical(3));
slope = stats_out.Coefficients{2,2}*tmp_table.trial_char;
ypred1 = slope.*inds1+stats_out.Coefficients{1,2}*inds1;
ypred2 = slope.*inds2+(stats_out.Coefficients{3,2}+stats_out.Coefficients{1,2})*inds2;
ypred3 = slope.*inds3+(stats_out.Coefficients{4,2}+stats_out.Coefficients{1,2})*inds3;
ypred1(ypred1==0) = [];
ypred2(ypred2==0) = [];
ypred3(ypred3==0) = [];
ypred = [ypred1;ypred2;ypred3];
...
stats_xlsx(cnts).mod_preds_stat = cell2csv_util(stats_out.Coefficients.tStat);
stats_xlsx(cnts).mod_preds_coeff = cell2csv_util(stats_out.Coefficients.Estimate);
stats_xlsx(cnts).multi_comp_terms = cell2csv_util(multi_comps_gn);
stats_xlsx(cnts).multi_comp_t1_t2 = cell2csv_util(multi_comps_t1_t2);
stats_xlsx(cnts).multi_comp_p = cell2csv_util(multi_comps_p);
stats_xlsx(cnts).multi_comp_coeff = cell2csv_util(multi_comps_f);
% stats_xlsx(cnts).multi_comp_lci_uci = cell2csv_util([comparisons(:,3),comparisons(:,5)]);
stats_xlsx(cnts).mod_r2 = cell2csv_util(stats_out.Rsquared.Adjusted);
stats_xlsx(cnts).norm_test_p = norm_p;
stats_xlsx(cnts).norm_test_h = norm_h;
stats_xlsx(cnts).effect_size = [alt_f2null,alt_f2t,alt_f2g];
stats_xlsx(cnts).effect_size_calc = 'alt_f2null,f2trial,f2group'; 
%- populate struct (2)
stats_struct(cnts).stat_test_mod = mod_out;
stats_struct(cnts).measure_tag = categorical(meas_names(meas_i));
stats_struct(cnts).design_tag = categorical(des_i); %designs(des_i);
% stats_struct(cnts).group_tag = categorial(g_i); %groups(g_i);
...
stats_struct(cnts).multi_comp_terms = multi_comps_gn;
stats_struct(cnts).multi_comp_t1_t2 = multi_comps_t1_t2;
stats_struct(cnts).multi_comp_p = multi_comps_p;
stats_struct(cnts).multi_comp_coeff = multi_comps_f;
% stats_struct(cnts).multi_comp_lci_uci = ([comparisons(:,3),comparisons(:,5)]);
stats_struct(cnts).mod_r2 = (stats_out.Rsquared.Adjusted);
stats_struct(cnts).norm_test_p = norm_p;
stats_struct(cnts).norm_test_h = norm_h;
stats_struct(cnts).effect_size = [alt_f2null,alt_f2t,alt_f2g];
stats_struct(cnts).effect_size_calc = 'ftnull,f2trial,f2group'; %'(R2_full-R2_null)/(1-R2_full)';
cnts = cnts + 1;
stats_struct(cnts) = DEF_STATS_TRACK_STRUCT;
stats_xlsx(cnts) = DEF_STATS_TRACK_XLSX;
%## PLOT =============================================================== %%
ta =  anova_out.pValue(strcmp(anova_out.Term,'trial_char'));
ta = {ta,ta,ta};
ga =  anova_out.pValue(strcmp(anova_out.Term,sprintf('%s',GROUP_TABLE_VAR)));
ga = {ga,ga,ga};
% gta =  anova_out.pValue(strcmp(anova_out.Term,sprintf('trial_char:%s',GROUP_TABLE_VAR)));
% ta = {gta,gta,gta};
% ga = {gta,gta,gta};
rs = double(stats_out.Coefficients.pValue(strcmp(stats_out.Coefficients.Name,'trial_char')));
int = double(stats_out.Coefficients.Estimate(strcmp(stats_out.Coefficients.Name,'(Intercept)')));
tm = double(stats_out.Coefficients.Estimate(strcmp(stats_out.Coefficients.Name,'trial_char')));
gint1 = double(stats_out.Coefficients.Estimate(strcmp(stats_out.Coefficients.Name,sprintf('%s_2',GROUP_TABLE_VAR))));
gint2 = double(stats_out.Coefficients.Estimate(strcmp(stats_out.Coefficients.Name,sprintf('%s_3',GROUP_TABLE_VAR))));
% gm1 = double(stats_out.Coefficients.Estimate(strcmp(stats_out.Coefficients.Name,sprintf('trial_char:%s_2',GROUP_TABLE_VAR))));
% gm2 = double(stats_out.Coefficients.Estimate(strcmp(stats_out.Coefficients.Name,sprintf('trial_char:%s_3',GROUP_TABLE_VAR))));
%## MAIN EFFECTS
coeffs = {[int,tm],...
        [int+gint1,tm],...
        [int+gint2,tm]};
...
%     sprintf('%s %0.2gx+%0.2g',ast,tm,int+gint1),...
%     sprintf('%s %0.2gx+%0.2g',ast,tm,int+gint2)};
...
STATS_STRUCT = struct('anova',{ta},...
    ...
    'pvals_pairs',{{}},...
    'pvals_grp',{num2cell(adj_p)},...
    'pvals_grp_pairs',{num2cell(multi_comps_t1_t2,2)},...
...
stats_out = fitlme(tmp_table,mod_out);
anova_out = anova(stats_out);
% [p1,F1,DF1,DF2] = coefTest(stats_out,[0,0,-1,0,-1,0]); % group 1 compared to group 2
% [p2,F2,DF1,DF2] = coefTest(stats_out,[0,0,0,-1,0,-1]); % group 1 compared to group 3
% [p3,F3,DF1,DF2] = coefTest(stats_out,[0,0,1,-1,1,-1]); % group 2 compared to group 3
[p1,F1,DF1,DF2] = coefTest(stats_out,[1,-1,0,1,-1,0]); % group 1 compared to group 2
[p2,F2,DF1,DF2] = coefTest(stats_out,[1,0,-1,1,0,-1]); % group 1 compared to group 3
[p3,F3,DF1,DF2] = coefTest(stats_out,[0,1,-1,0,1,-1]); % group 2 compared to group 3
[h,crit_p,adj_ci_cvrg,adj_p] = fdr_bh([p1,p2,p3]);
% [p1,F1,DF1,DF2] = coefTest(stats_out,[0,1,-1,0]); % group 1 compared to group 2
% [p2,F2,DF1,DF2] = coefTest(stats_out,[0,1,0,-1]); % group 1 compared to group 3
% [p3,F3,DF1,DF2] = coefTest(stats_out,[0,0,1,-1]); % group 2 compared to group 3
% [h,crit_p,adj_ci_cvrg,adj_p] = fdr_bh([p1,p2,p3]);
multi_comps_p = adj_p;
multi_comps_f = [F1,F2,F3];
...
stats_xlsx(cnts).norm_test_h = norm_h;
stats_xlsx(cnts).effect_size = [alt_f2null,alt_f2t,alt_f2g];
stats_xlsx(cnts).effect_size_calc = 'f2trial,f2group'; 
%- populate struct (2)
stats_struct(cnts).stat_test_mod = mod_out;
stats_struct(cnts).measure_tag = categorical(meas_names(meas_i));
stats_struct(cnts).design_tag = categorical(des_i); %designs(des_i);
...
stats_struct(cnts).norm_test_h = norm_h;
stats_struct(cnts).effect_size = [alt_f2null,alt_f2t,alt_f2g];
stats_struct(cnts).effect_size_calc = 'f2null,f2trial,f2group'; %'(R2_full-R2_null)/(1-R2_full)';
cnts = cnts + 1;
stats_struct(cnts) = DEF_STATS_TRACK_STRUCT;
stats_xlsx(cnts) = DEF_STATS_TRACK_XLSX;
%## PLOT =============================================================== %%
...
stats_xlsx(cnts).norm_test_h = norm_h;
stats_xlsx(cnts).effect_size = [alt_f2null,alt_f2t,alt_f2g];
stats_xlsx(cnts).effect_size_calc = 'f2null,f2trial,f2group'; 
%- populate struct (2)
stats_struct(cnts).stat_test_mod = mod_out;
stats_struct(cnts).measure_tag = categorical(meas_names(meas_i));
stats_struct(cnts).design_tag = categorical(des_i); %designs(des_i);
...
stats_struct(cnts).norm_test_h = norm_h;
stats_struct(cnts).effect_size = [alt_f2null,alt_f2t,alt_f2g];
stats_struct(cnts).effect_size_calc = 'alt_f2null,f2trial,f2group'; %'(R2_full-R2_null)/(1-R2_full)';
cnts = cnts + 1;
stats_struct(cnts) = DEF_STATS_TRACK_STRUCT;
stats_xlsx(cnts) = DEF_STATS_TRACK_XLSX;
%## PLOT =============================================================== %%
...
%-
save([save_dir filesep 'figure_behavioral_data_stats_anova.mat'],'stats_xlsx')
stats_xlsx = struct2table(stats_xlsx);
stats_xlsx(isundefined(stats_xlsx.measure_tag),:) = [];
% stats_struct(:,'anova_preds_terms')
writetable(stats_xlsx,[save_dir filesep 'figure_behavioral_data_stats_lmeanova.xlsx'])