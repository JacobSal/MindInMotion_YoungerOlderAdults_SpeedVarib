...
PLOT_STRUCT = struct( ...
    ...
    'xlim',[CONN_FREQS(1),CONN_FREQS(end)],...
    'x_label',{'Frequency (Hz)'},...
    'x_label_props',{{ ...
        'Units','Normalized', ...
        'FontSize',8, ...
...
%% (ALL SUBJS MODEL) =================================================== %%
meas_ext = 'combo';
tmp_savedir = [save_dir filesep fextr '_psdvio_manufigs_new'];
mkdir(tmp_savedir);
%--
FIG_INIT_Y = 0;
FIG_INIT_X = 0;
...
for cl_i = 1:length(cluster_inds_plot)
    ...
    % params.line_plot_opt = 'inter';
    % params.line_plot_opt = 'cond';
    % params.xtick_label = g_chars_subp;
    params.xtick_label = xtick_label_g;
    params.xtick_label_g = g_chars_subp;
    params.xtick_label_c = xtick_label_g;
    params.freqs = fooof_freqs;
    params.line_styles = {'-','-.','--'};
    %--
    % cg = linspecer(length(g_chars_subp));
    % tmpg = repmat(linspecer(length(g_chars_subp)),[1,1,size(cmaps_speed,1)]);
    % tmpc = repmat(cmaps_speed,[1,1,length(g_chars_subp)]);
    % cat(1,cg,cmaps_speed)
    ...
    leg_store = [leg_store; paramso.stats_store];
    %--
    % des_i = 2;
    % %## EXTRACT STATS INFO
    % paramss = [];        
    % paramss.group_chars = {'H1000','H2000','H3000'};
    % paramss.group_order = categorical({'H1000','H2000','H3000'});
    % paramss.model_char_int = 'speed_group_intact_all';
    % paramss.model_char_group = 'speed_group_all';
    % paramss.group_char = 'all';
    % paramss.kin_measure = 'none';
    % paramss.anv_chars_int = ANV_CHARS_INT;
    % paramss.anv_chars_group = ANV_CHARS_GROUP;
    % paramss.coeff_chars_int = COEFF_CHARS_INT;
    % paramss.coeff_chars_group = COEFF_CHARS_GROUP;
    % %--
    % % params = [];
    % params.group_chars = {'H1000','H2000','H3000'};
    % params.violin_measures = {'std_avg_theta_fn1', ...
    %     'std_avg_beta_fn1'};
    % params.vio_x_vals = repmat([(8-3)/2 + 3,(30-13)/2 + 13],[gsz,1]);
    % params.vio_x_vals(2,:) = params.vio_x_vals(2,:)+1;
    % params.vio_x_vals(3,:) = params.vio_x_vals(3,:)+2;
    % inds = strcmp(KIN_TABLE.model_n,num2str(des_i)) & ...
    %         KIN_TABLE.cluster_n == cl_n;
    % kin_t_in = KIN_TABLE(inds,:);
    % %## NORMALIZE DATA USING SUBJECT INTERCEPTS
    % % if ~isempty(ranef.char)
    % %     for e_i = 1:length(params.violin_measures)
    % %         eeg_measure = params.violin_measures{e_i};
    % %         paramss.eeg_measure = eeg_measure;
    % %         %--
    % %         [STATS_STRUCT,CONFINT_STRUCT,ranef] = extract_violin_stats(RSTATS_IMPORT,cl_n,paramss);
    % %         for s_i = 1:length(ranef.char)
    % %             int = ranef.int(s_i);
    % %             ind = strcmp(ranef.char{s_i},kin_t_in.subj_char);
    % %             kin_t_in(ind,eeg_measure) = kin_t_in(ind,eeg_measure)-int;
    % %         end
    % %     end
    % % end
    % params.stats_ax_pos = [x_shift+AX_W*IM_RESIZE + 0.01,y_shift,AX_W*IM_RESIZE*0.1,AX_H*IM_RESIZE];
    % %--
    % ax = axes();
    % [paramso] = local_psd_plot_inter_vio(ax,psd_dat_in,kin_t_in,params,tmp_plot_struct,tmp_line_struct);
    %--
    if ~exist([tmp_savedir filesep fname],'file') || DO_RECALC_STATS
    ...
    leg_store = [paramso.leg_store;leg_store];
    xlabel(ax,'Frequency (Hz)');
    ylabel(ax,'<<10*log_{10}(PSD_{N})>>');

    %##
    params.leg_position = [-0.225,0.07];
    %-- legend
    ...
    EEG_MEASURE_TITLES = {'', ...
        ''};
    EEG_MEASURE_LABS = {'<log_{10}(COV(\theta_{N}))>', ...
        '<log_{10}(COV(\beta_{N}))>'};
    %--
    x_shift = AX_INIT_X;   
    y_shift = AX_INIT_Y;
    ...
    for e_i = 1:length(EEG_MEASURES)
        ...
        % params = [];        
        % params.group_chars = {'H1000','H2000','H3000'};
        % % params.group_chars = {'YA','OHFA','OLFA'}; %{'OHFA','OLFA','YA'};
        % % params.group_chars = {'OHFA','OLFA','YA'}; %{'OHFA','OLFA','YA'};
        % params.group_order = categorical({'H1000','H2000','H3000'});
        % % params.group_order = categorical({'YA','OHFA','OLFA'});
        % % params.group_order = categorical({'OHFA','OLFA','YA'});
        ...
        %--
        % [STATS_STRUCT,CONFINT_STRUCT,ranef] = extract_violin_stats(RSTATS_IMPORT,cl_n,params);
        [STATS_STRUCT,CONFINT_STRUCT,ranef] = get_r_stats_func(RSTATS_IMPORT,cl_n, ...
        ...
        ax = axes();
        %-- set parameter
        VIO_PLOT_STRUCT.group_labels = g_chars_subp; %params.group_chars; %g_chars_subp;
