...
%-- r stats
fextr = 'meansd_new_meancondb_nfslidingb6'; % 013022025_lme_eeg_kin_meansd_new_meancondb_nfslidingb6_stats
%-- r table
...
% RSTATS_IMPORT = readtable([r_stats_dir filesep '013022025_lme_eeg_kin_meansd_new_meancondb_nfslidingb6_stats'], ...
%     "FileType","spreadsheet","UseExcel",true);
%-- r-table
KIN_TABLE = readtable([r_stats_dir filesep sprintf('01302025_lme_eeg_kin_%s_tbl.xlsx',fextrt)], ...
    "FileType","spreadsheet","UseExcel",true);
X_DIM = 2;
%% (ALTERNATIVE TABLE LOAD) 
% r_stats_tbl = readcell([r_stats_dir filesep sprintf('01302025_lme_eeg_kin_%s_stats.xlsx',fextr)], ...
%     'FileType','spreadsheet', ...
%     'sheet','Sheet 1');
% r_stats_tbl =r_stats_tbl(2:end,1:21);
% tbl_sz = size(r_stats_tbl);
% inds = cellfun(@(x) ismissing(x),r_stats_tbl,'UniformOutput',false);
% inds = cellfun(@(x) chk_inds(x),inds);
% r_stats_tbl(inds) = {''};
% r_stats_tbl = reshape(r_stats_tbl,tbl_sz);
% r_stats_tbl = cell2table(r_stats_tbl);
% r_stats_tbl.Properties.VariableNames(1:21) = {'SortingNum','SubjCode','SP_0p5_1',...
...
%% (EXAMPLE) =========================================================== %%
% %--
% peak_params = [17.0406,0.7288,2.5539; ...
%     20.3958,0.6000,1.6476; ...
%     25.5860,0.7712,3.8497];
% % gaussian_params = [17.0406,0.7290,1.2770; ...
% %    20.3958,0.5624,0.8238; ...
% %    25.5860,0.7719,1.9248];
...
% figure;
% hold on;
% modl = fitlm(xx,fooof_mod((xx),1.08,-1.584,0,A,W,C));
% plot(xx,fooof_mod((xx),1.08,-1.584,0,A,W,C),'DisplayName','0.25 m/s') % slow speeds
% plot(xx,modl.Coefficients.Estimate(1)+modl.Coefficients.Estimate(2)*xx)
% %--
% modl = fitlm(xx,fooof_mod((xx),1.167,-1.392,0,A,W,C));
% plot(xx,fooof_mod((xx),1.167,-1.392,0,A,W,C),'DisplayName','0.5 m/s') % faster speeds
% plot(xx,modl.Coefficients.Estimate(1)+modl.Coefficients.Estimate(2)*xx)
% %--
% modl = fitlm(xx,fooof_mod((xx),1.194,-1.329,0,A,W,C));
% plot(xx,fooof_mod((xx),1.194,-1.329,0,A,W,C),'DisplayName','0.75 m/s')
% plot(xx,modl.Coefficients.Estimate(1)+modl.Coefficients.Estimate(2)*xx)
% %--
% modl = fitlm(xx,fooof_mod((xx),1.21,-1.289,0,A,W,C));
% plot(xx,fooof_mod((xx),1.21,-1.289,0,A,W,C),'DisplayName','1.0 m/s')
% plot(xx,modl.Coefficients.Estimate(1)+modl.Coefficients.Estimate(2)*xx)
% xlim([min(xx),max(xx)]);
% legend();
% % freqfoof = @(x,b,c) c.*(x.^(-b));
...
for cl_i = 1:length(clusters)    
    ...
    for kin_i = 1:length(KINNAMES)
        ...
        exportgraphics(fig,[tmp_savedir filesep sprintf('intacteff_cl%s_kin%s_plot_allsubj.tiff',string(clusters(cl_i)),KINNAMES{kin_i})],...
            'Resolution',600)
        % close(fig)
    end
end
