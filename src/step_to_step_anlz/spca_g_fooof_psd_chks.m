...
subj_chars = unique(FOOOF_TABLE.subj_char);
cluster_inds = unique(FOOOF_TABLE.cluster_id);
g_chars = unique(FOOOF_TABLE.group_char);
des_i = 3;
cond_i = 1;
%-
tmpd = dir([spca_dir filesep STUDY.datasetinfo(subj_i).subject filesep 'GAIT_EPOCHED' filesep '*' filesep sprintf('cond%s_spca_psd.mat',condition_gait{cond_i})]);
...
for cc = 1:length(cluster_inds)
    ...
    for ss = 1:length(s_chars)
        ...
        %- store
        FOOOF_TABLE.speed_ms(cnt) = cl_speeds;
        FOOOF_TABLE.subj_id(cnt) = subj_inds;
        FOOOF_TABLE.subj_cl_ind(cnt) = cl_inds;
        FOOOF_TABLE.subj_char(cnt) = categorical(cl_chars);
        FOOOF_TABLE.comp_id(cnt) = cl_comps;
        FOOOF_TABLE.design_id(cnt) = categorical(des_i);
        FOOOF_TABLE.cond_char(cnt) = categorical({'rest'});
        FOOOF_TABLE.cond_id(cnt) = cond_i;
        FOOOF_TABLE.speed_diff_stat(cnt) = 0;
        FOOOF_TABLE.speed_div_stat(cnt) = 0;
        FOOOF_TABLE.speed_diffdiv_stat(cnt) = 0;       
        FOOOF_TABLE.group_id(cnt) = group_i;
        try
            if isnan(g_chars)
                FOOOF_TABLE.group_char(cnt) = categorical({'all'});
        ...
        FOOOF_TABLE.cluster_id(cnt) = categorical(cl_i);
        FOOOF_TABLE.aperiodic_exp(cnt) = fooof_results.aperiodic_params(2);
        FOOOF_TABLE.aperiodic_offset(cnt) = fooof_results.aperiodic_params(1);
        FOOOF_TABLE.central_freq{cnt} = fooof_results.peak_params(:,1);
        FOOOF_TABLE.power{cnt} = fooof_results.peak_params(:,2);
        % try
        %     FOOOF_TABLE.central_freq{cnt} = fooof_results{des_i}{cond_i,group_i}{subj_i}.peak_params(1,:);
        %     FOOOF_TABLE.power{cnt} = fooof_results{des_i}{cond_i,group_i}{subj_i}.peak_params(2,:);
        % catch
        %     fprintf('No peaks generated for subject: %s...\n',cl_chars{subj_i});
        % end
        FOOOF_TABLE.r_squared(cnt) = fooof_results.r_squared;
        %- Compute average power after flatten curve
