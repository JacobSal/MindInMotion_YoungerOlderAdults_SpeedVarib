...
% KINNAMES_LABS = {'Step Width (m)','AP Exc. (m)','Step Width GC (m)','AP Exc. GC (m)'};
KINNAMES = {'step_width_mm','ap_exc_mm','step_width_mm_gc','ap_exc_mm_gc'};
...
%% (ALL SUBJS MODEL) =================================================== %%
tmp_savedir = [save_dir filesep '2dplot_intact_all'];
mkdir(tmp_savedir);
grp_i = 1;
for cl_i = 1:length(clusters)
    %##
    for kin_i = 1:length(KINNAMES)
        ...
        hold off;
        %##
        exportgraphics(fig,[tmp_savedir filesep sprintf('cl%s_kin%s_plot_allsubj.tiff',string(clusters(cl_i)),KINNAMES{kin_i})],'Resolution',300)
        close(fig)
    end
end
%% ===================================================================== %%
