...
%-
KINNAMES = {'step_width_gc_mm','ap_exc_gc_mm'};
...
RSTATS_IMPORT = readtable([r_stats_dir filesep 'ersp_kin_gc_meas_nonorm_0p005cut.xlsx'], ...
    "FileType","spreadsheet","UseExcel",true);
%% ===================================================================== %%
...
%## ALPHA & BETA
IM_RESIZE = 1;
X_DIM = 2; % number of measures
REG_TXT_SIZE = 7; % 7
REG_X_SHIFT = 0.1; % 0.08
REG_Y_SHIFT = 0.09; % 0.1k
AX_TXT_SIZE = 10;
...
for cl_i = 1:length(clusters)
    ...
    for kin_i = 1:length(KINNAMES)
        ...
        for meas_i = 1:length(EEG_MEASURES)
            ...
            tmp_tbl = tmp_kint(inds&inds_keep,:);
            ...
            %- normalize subject data using random intercept
            ...
            for cond_i = 1:length(speed_ns)
                ...
                data = data(inds,:);
                ss = scatter(data,KINNAMES{kin_i},EEG_MEASURES{meas_i}, ...
                    'DisplayName',sprintf('%0.2f',speed_ns(cond_i)));
                ...
                if anova_p_intact < ALPHA
                    ...
                    if DO_PLOT_R2
                        ...
                        if ~isempty(chkl) & cond_i == 1
                            ...
                            str = sprintf('%s R^2=%0.2g\ny_{0.25}=(%0.1g)x+(%0.1g)\ny_{1.0}=(%0.1g)x+(%0.1g)',ast,R2,m_25,b_25,m_1,b_1);
        ...
        exportgraphics(fig,[tmp_savedir filesep sprintf('intacteff_cl%s_kin%s_plot_allsubj.tiff',string(clusters(cl_i)),KINNAMES{kin_i})],'Resolution',SAVE_RES)
        close(fig)
    end
end
%% ===================================================================== %%
