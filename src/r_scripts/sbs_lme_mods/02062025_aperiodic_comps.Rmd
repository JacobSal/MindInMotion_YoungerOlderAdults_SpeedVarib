...
kin_measures = c('mu_ml_exc_mm_gc_fn1','mu_step_dur_fn1',
                 'mu_ml_exc_mm_gc_fn2','mu_step_dur_fn2');
kin_title_chars = c("ml exc. mean","step dur. mean",
                    "ml exc. std dev.","step dur. std dev.")
...
theme_set(theme_classic()) # This sets the default ggplot theme
#%% LOOP VARS
cis <- rep(NA,length(clusters)*length(eeg_measures)*length(tbl_groups)*length(tbl_etypes))
eis <- rep(NA,length(clusters)*length(eeg_measures)*length(tbl_groups)*length(tbl_etypes))
gis <- rep(NA,length(clusters)*length(eeg_measures)*length(tbl_groups)*length(tbl_etypes))
epis <- rep(NA,length(clusters)*length(eeg_measures)*length(tbl_groups)*length(tbl_etypes))
cnt = 1;
for (i in 1:length(clusters)) {
  for(k in 1:length(eeg_measures)){
...
for (i in 1:length(cis)){
  ...
  #%% PRINT TABLE
  t_out <- as_gt(tbl) %>%
    gt::tab_header(title=c("Changes in ",ei,"(",gi,")","for Cluster: ",ci)) %>%
    gt::tab_options(table.layout = "fixed") %>%
    gt::as_raw_html()
  print(t_out)
  #-- extract fixed & random effects coeffs
...
```
# EEG-SPEED ALL) LME EEG ~ 1+speed+epochtype+epochtype:speed
```{r eegspeedallintactept, echo=FALSE, message=FALSE, results="asis"}
theme_set(theme_classic()) # This sets the default ggplot theme
#%% LOOP VARS
...
for (i in 1:length(cis)){
  ...
  #%% COMPUTE LME MODEL
  mod_char = paste(ei," ~ 1 + speed_cond_num + epoch_type + speed_cond_num:epoch_type + (1|subj_char)");
  mod <- lme4::lmer(str_glue(mod_char), data=tmpt);
  tbl <- mod %>%
    tbl_regression(tidy_fun = broom.mixed::tidy,
                   pvalue_fun = purrr::partial(style_pvalue, digits = 2),
...
theme_set(theme_classic()) # This sets the default ggplot theme
#%% LOOP VARS
cis <- rep(NA,length(clusters)*length(kin_measures)*length(eeg_measures)*length(tbl_etypes))
kis <- rep(NA,length(clusters)*length(kin_measures)*length(eeg_measures)*length(tbl_etypes))
eis <- rep(NA,length(clusters)*length(kin_measures)*length(eeg_measures)*length(tbl_etypes))
epis <- rep(NA,length(clusters)*length(kin_measures)*length(eeg_measures)*length(tbl_etypes))
cnt = 1;
for (i in 1:length(clusters)) {
  for(j in 1:length(kin_measures)){
    for(k in 1:length(eeg_measures)){
      for(l in 1:length(tbl_etypes)){
        cis[cnt] = i;
        kis[cnt] = j;
        eis[cnt] = k;
        epis[cnt] = l;
...
for (i in 1:length(cis)){
  ...
  new_row = data.frame(cluster_num=ci,
                      group_char=c('all'),
                      model_char=c('all_ap_kin_intact'),
                      kinematic_char=c(ki),
                      freq_band_char=c(ei),
                      coeff_int=mod@beta[1],
                      coeff_sp=mod@beta[2],
...
write.xlsx(excel_df,paste0("013022025_lme_eeg_aperid_",fext,".xlsx"))
```
