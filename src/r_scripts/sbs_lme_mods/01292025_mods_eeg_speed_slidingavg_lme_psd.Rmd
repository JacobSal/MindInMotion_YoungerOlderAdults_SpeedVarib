...
```{r errordat}
num_strides_avg = 5;
conds = unique(tmpts$cond_char);
#%% REMOVE ERRONEOUS DATA IN KINEMATICS
dtbl <- eegt
newtbl <- dtbl[0,];
subjects = unique(dtbl$subj_char);
orig_len = nrow(dtbl)

#-- loop through subjects
for(k in 1:length(tmpt_subjects)){
  tmpts <- filter_at(dtbl,vars('subj_char'), any_vars(. %in% subjects[k]));
  cond_chars <- unique(tmpts$cond_char)
  print(sprintf('Processing subject %s...\n',subjects[k]))
  for( j in 1:length(cond_chars)){
    tmptc <- filter_at(tmpts,vars('cond_char'), any_vars(. %in% cond_chars[j]));
    for( i in 1:length(clusters)){
      tmptcc <- filter_at(tmptc,vars('cluster_n'), any_vars(. %in% clusters[i]));
      n_strides = length(unique(tmpts$stride_n))
      #-- iterate through condition and average strides.
      cnt = 1
      while( cnt < n_strides){
        #%% reduce table and get specific subject data?
        tt <- tmptcc[seq(cnt,cnt+num_strides_avg-1,by=1),]
        tt <- tt %>%
          group_by(subj_char,cond_char,speed_n,model_n,group_char,group_name,model_char,cluster_n) %>%
          summarise_all("mean")
        newtbl <- rbind(newtbl,tt)
        cnt = cnt + num_strides_avg;
      }
      if(cnt!=n_strides){
        cnt = cnt - num_strides_avg;
        tt <- tmptcc[seq(cnt,n_strides,by=1),]
        tt <- tt %>%
          group_by(subj_char,cond_char,speed_n,model_n,group_char,group_name,model_char,cluster_n) %>%
          summarise_all("mean")
        newtbl <- rbind(newtbl,tt)
...
new_len = nrow(newtbl)
print(sprintf("Kinematics) Number of data points condensed: %d",orig_len-new_len))
write.xlsx(newtbl,"lme_eeg_kin_speed_slidingavg_tbl.xlsx")
dtbl <- newtbl; 
rm(newtbl)

#%% LOAD SHEET
# excel_dir <-"/jsalminen/GitHub/MIND_IN_MOTION_PRJ/MindInMotion_YoungerOlderAdult_KinEEGCorrs/src/r_scripts/sbs_lme_mods/lme_eeg_kin_speed_slidingavg_tbl.xlsx"
# 
# if(ispc()){
#   excel_dir <- paste0("M:",excel_dir)
# }else{
...
# }
# 
# dtbl <- read_excel(excel_dir,sheet="Sheet 1")
# rm(orig_eegt)
# rm(eegt)
...
dtbl <- mutate(dtbl,across(c('group_name'), factor))
dtbl <- mutate(dtbl,across(c('model_char'), factor))
...
# std_cutoff_eeg = 4;
# std_cutoff_kin = 3.5;
# kin_measures = c('ml_exc_mm_gc','step_dur','stance_dur');
# eeg_measures = c('avg_theta_post','avg_alpha_post','avg_beta_post');
# #%% REMOVE ERRONEOUS DATA IN KINEMATICS
# dtbl <- eegt
# newtbl <- dtbl[0,];
# tmpt_subjects = unique(dtbl$subj_char);
# orig_len = nrow(dtbl)
# 
# #-- loop through subjects
# for(k in 1:length(tmpt_subjects)){
...
#     ki = kin_measures[i];
#     #%% reduce table and get specific subject data?
#     tmpts <- filter_at(dtbl,vars('subj_char'), any_vars(. %in% tmpt_subjects[k]));
#     
#     #%% THAT ARE GREATER THAN OR LESS THAN 3STD FROM MEAN
#     #-- remove NAN cases
#     tmpts = tmpts[complete.cases(tmpts[[ki]]),];
#     #- mean & std cutoff
#     muc = mean(tmpts[[ki]])
#     stdc = sd(tmpts[[ki]])
#     inds_keep = tmpts[[ki]] > muc-std_cutoff_kin*stdc & tmpts[[ki]] < muc+std_cutoff_kin*stdc
#     tmpts = tmpts[inds_keep,]
#     newtbl <- rbind(newtbl,tmpts)
#   }
# }
# new_len = nrow(newtbl)
# print(sprintf("Kinematics) Number of data points lost: %d",orig_len-new_len))
# 
# #%% REMOVE DATA BASED ON EEG MEASURES
# #-- reassign
# # dtbl <- eegt # if not using kin cutoff
# dtbl <- newtbl
# orig_len = nrow(dtbl)
# newtbl <- dtbl[0,];
# 
...
#   tmpts <- filter_at(dtbl,vars('subj_char'), any_vars(. %in% tmpt_subjects[k]));
#   for (i in 1:length(clusters)) {
#     tmptc <- filter_at(tmpts,vars('cluster_n'), any_vars(. %in% clusters[i]));
#     for(j in 1:length(eeg_measures)){
#       ei = eeg_measures[j];
#       
#       #%% THAT ARE GREATER THAN OR LESS THAN 3STD FROM MEAN
#       #-- remove NAN cases
#       tmptc = tmptc[complete.cases(tmptc[[ei]]),];
#       #- mean & std cutoff
#       muc = mean(tmptc[[ei]])
#       stdc = sd(tmptc[[ei]])
#       inds_keep = tmptc[[ei]] > muc-std_cutoff_eeg*stdc & tmptc[[ei]] < muc+std_cutoff_eeg*stdc
#       tmptc = tmptc[inds_keep,]
#       newtbl <- rbind(newtbl,tmptc)
#     }
#   }
# }
# new_len = nrow(newtbl)
# print(sprintf("EEG) Number of data points lost: %d",orig_len-new_len))
```
<!-- # Table Manipulations -->
<!-- ```{r tblmanip} -->
...
<!-- dtbl <- transform(dtbl,cov_avg_beta_post = avg_beta_post_fn2/avg_beta_post_fn1) -->
<!-- dtbl <- transform(dtbl,cov_avg_alpha_post = avg_alpha_post_fn2/avg_alpha_post_fn1) -->
<!-- dtbl <- transform(dtbl,cov_ml_exc = ml_exc_mm_gc_fn2/ml_exc_mm_gc_fn1) -->
<!-- dtbl <- transform(dtbl,cov_step_dur = step_dur_fn2/step_dur_fn1) -->
<!-- dtbl <- transform(dtbl,cov_stance_dur = stance_dur_fn2/stance_dur_fn1) -->
...
<!--                      'avg_beta_post_fn1','avg_beta_post_fn2','cov_avg_beta_post'); -->
<!-- eeg_title_chars = c("**THETA** Mean","**THETA** Std. Dev.","**THETA** COV", -->
<!--                          "**BETA** Mean","**BETA** Std. Dev.","**BETA** COV"); -->
<!-- #-- CLUSTERS TO PLOT -->
<!-- clusters = c(3,4,6,7,8,9,10,13) -->

<!-- #%% MUTATE VARIABLES -->
<!-- dtbl$speed_cond_num <- as.numeric(dtbl$speed_n); -->
<!-- write.xlsx(dtbl,"lme_eeg_kin_mean_sd_speedeeg_kin_anl_tbl.xlsx") -->
<!-- dtbl <- dtbl; -->
<!-- ``` -->
...
# write.xlsx(excel_df,"lme_eeg_kin_mean_sd_stats_eegspeed_anl.xlsx")
```
...
write.xlsx(excel_df,"lme_eeg_kin_raw_stats.xlsx")
```
