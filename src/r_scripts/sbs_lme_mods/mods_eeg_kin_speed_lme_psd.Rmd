...
# MODERATION 2, ALL) LME EEG ~ 1+speed+kin+speed:kin
```{r eegspeedkininterall, echo=FALSE, message=FALSE, results="asis"}
ci <- rep(NA,length(clusters)*length(kin_measures)*length(eeg_measures))
ki <- rep(NA,length(clusters)*length(kin_measures)*length(eeg_measures))
ei <- rep(NA,length(clusters)*length(kin_measures)*length(eeg_measures))
cnt = 1;
for (ci in 1:length(clusters)) {
  for(ki in 1:length(kin_measures)){
    for(ei in 1:length(eeg_measures)){
      ci[cnt] = ci;
      ki[cnt] = ki;
      ei[cnt] = ei
      cnt = cnt + 1;
