function [] = mim_imu_get_strides(EEG_IMU,varargin)
%MIM_IMU_GET_STRIDES Summary of this function goes here
%   IN: 
%   OUT: 
%   IMPORTANT
%
...
gaitEvents = EEG_IMU.event( strcmpi('RHS',{EEG_IMU.event.type}) | strcmpi('RTO',{EEG_IMU.event.type}) | strcmpi('LHS',{EEG_IMU.event.type}) | strcmpi('LTO',{EEG_IMU.event.type})); %grab just right heel strike info
...
for stride_i = 1:(nStrides)
   ...
   % indGaitEventsInThisSTride = []; ContainedEventTypes = [];
    ...
    tStartStride = (RHSevents(stride_i).latency-1)/EEG_IMU.srate;
    tEndStride = (RHSevents(stride_i+1).latency-1)/EEG_IMU.srate;  
    if DoCrop
    ...
    tInd = find(EEG_IMU.times/1000 >= tStartStride & EEG_IMU.times/1000 <= tEndStride);
    if ~isempty(tInd)
        tempData = EEG_IMU.data(:,tInd); %grab data for this stride
