function [STUDY,comps_store,comps_rej] = cluster_iclabel_reduce(STUDY,ALLEEG,varargin)
    ...
    for i = 1:length(tmpsets)
        ...
        try
            if ~isfield(ALLEEG(subj_i).etc.ic_classification.ICLabel,'classifications')
                ALLEEG(subj_i) = iclabel(ALLEEG(subj_i));
            end
        catch e
            fprintf('Error. Subject %s\n%s\n',ALLEEG(subj_i).subject,getReport(e));
            ALLEEG(subj_i) = iclabel(ALLEEG(subj_i));
        end
        %- choosing component based on minimum dipole fit residual variance
        if ~isempty(comps_clust) && length(comps_clust) > 1
            clssts = ALLEEG(subj_i).etc.ic_classification.ICLabel.classifications;
            [val,idx] = max(clssts(comps_clust,1));
