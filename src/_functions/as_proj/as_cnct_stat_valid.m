function [conn_store_subj] = as_cnct_stat_valid(EEG,STUDY,cluster_iters,cluster_names,component_vec,save_dir,varargin)
...
%                 end
                ...
                %- save figures
                if SAVE_FIG
                    saveas(fig_i,[save_dir filesep sprintf('%s_%s_valsshown-%s_TimeFreqChart_%s-%s.fig',EEG(1).subject,STAT_CHAR{stat_i},tmp(eeg_i).condition,cond_bootstrap{cond_i,cond_j}{1},cond_bootstrap{cond_i,cond_j}{2})]);
                    saveas(fig_i,[save_dir filesep sprintf('%s_%s_valsshown-%s_TimeFreqChart_%s-%s.jpg',EEG(1).subject,STAT_CHAR{stat_i},tmp(eeg_i).condition,cond_bootstrap{cond_i,cond_j}{1},cond_bootstrap{cond_i,cond_j}{2})]);
                    close(fig_i)
                end
            end
        end
