function [p] = mcc_gen_filepaths(d)
    ...
    if nargin==0
      p = mcc_gen_filepaths(fullfile(matlabroot,'toolbox'));
    ...
    %
    dirs = files; %files(isdir); % select only directory entries from the current listing
    
    for i=1:length(dirs)
        dirname = dirs(i).name;
        regchk = regexp(dirname,'(?<=\.)[^.]*$','match');
        if ~strcmp( dirname,'.')          && ...
           ~strcmp( dirname,'..')         && ...
           ...
           if isdir(i)
               p = [p pathsep mcc_gen_filepaths([d filesep dirname])]; % recursive calling of this function.
           elseif ~isempty(regchk)
               if strcmp(regchk,'m') ||...
                   strcmp(regchk,'mex')
                   if isempty(p)
                       p = [dirs(i).folder filesep dirname];
                   else
                       p = [p pathsep [dirs(i).folder filesep dirname]];
                   end
               end
           end
        end
