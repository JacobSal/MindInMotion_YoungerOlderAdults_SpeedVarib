...
addpath(SRC_DIR);%## (LOAD GAIT TIMEWARP) ======================================= %%
            ...
            GPM = bsxfun(@minus,ERSP,mean(E%## (LOAD GAIT TIMEWARP) ======================================= %%
            fprintf('Loading Gait Time-Frequency Data...\n');
            icatimef_f = [gait_fpath filesep sprintf('%s.icatimef',subj_char)];
            MEAN_STRUCT = struct('do_return',true, ...
                'fcn',@(x) squeeze(mean(abs(x),3)));
...
parfor subj_i = 1:length(STUDY.datasetinfo)
    ...
    try
        ...
        %- load spca data
        spca_ersp = par_load(tmp_spca_study.datasetinfo(subj_ii).filepath,sprintf('cond%s_spca_ersp.mat',gait_conds{1}));
        %- chk ic transforms
        ...
        %## EXTRACT SPCA ERSP DATA
        for cond_i = 1:length(gait_conds)
            spca_ersp = par_load(tmp_spca_study.datasetinfo(subj_ii).filepath,sprintf('cond%s_spca_ersp.mat',gait_conds{cond_i}));
            for i = 1:length(main_cl_inds)
                ...
                if ~isempty(comp_i) && length(comp_i) < 2
                    ...
                    spca_struct(cnt).tf_erspcorr_c = squeeze(spca_ersp.ersp_corr(:,tmp_c,:));
                    spca_struct(cnt).tf_gpmcorr_c = squeeze(spca_ersp.gpm_corr(:,tmp_c,:));
                    spca_struct(cnt).tf_ersporig_c = []; %squeeze(spca_ersp.ersp_uncorr(:,tmp_c,:));
                    spca_struct(cnt).tf_gpmorig_c = []; %squeeze(spca_ersp.gpm_uncorr(:,tmp_c,:));
                    spca_struct(cnt).tf_pc1_c = []; %squeeze(spca_ersp.pc1(:,tmp_c,:));
                    spca_struct(cnt).tf_coeff_c = []; %spca_ersp.coeffs;
                    %--
                    ...
                    spca_struct(cnt).group_c = tmp_study.datasetinfo(subj_i).group;
                    spca_struct(cnt).cluster_c = cl_i;
                    spca_struct(cnt).comp_c = sprintf('study_ic, %i; unmix_ic, %i; keep_ic, %i; cluster_ic, %i',comp_i,tmp_c,ic_keep(comp_i),comp_i);
                    cnt = cnt+1;
                end
            end
...
tt = struct2table(tt);
par_save(tt,cluster_k_dir,'spca_cluster_table_ersp.mat');
